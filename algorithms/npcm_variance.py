...
class npcm_variance():
    ...
    def init_theta_ita(self):
        ...
        self.cov_mat = np.delete(self.cov_mat, index_delete, axis=0)
        self.m -= p
    ...
    def update_u_theta(self):
        ...
        for cntr_index in range(self.m):
            ...
            # tmp_x_c = np.mat(self.x - self.theta[cntr_index])
            # cov_mat = self.cov_mat[cntr_index]
            # dist_2_cntr = np.sqrt(np.power(np.det(cov_mat), -self.d) * tmp_x_c * np.linalg.pinv(cov_mat) * tmp_x_c.T)
            tmp_x_c = self.x - self.theta[cntr_index]
            cov_mat = self.cov_mat[cntr_index]
            # dist_2_cntr = np.array(map(lambda x: mahalanobis(x, self.theta[cntr_index], cov_mat), self.x))\
            #               /np.sqrt(np.power(np.linalg.det(cov_mat),-self.d))
            dist_2_cntr = np.array(map(lambda x: mahalanobis(x, self.theta[cntr_index], cov_mat), self.x))
    ...
    def cluster_elimination(self):
        ...
        if p > 0:
            ...
            self.m -= p
            self.log.debug("list:%s", index_delete)
            self.log.debug("del Number:%d", p)
        labels = np.argmax(self.u, axis=1)
    ...
    def __call__(self, p):
        ...
        # remember to add the needs-to-update elments to the return list
        # return self.lines + self.line_centers + self.inner_circles + self.circles + [self.text] + self.outer_circles
        return self.lines + self.line_centers + [self.text]
