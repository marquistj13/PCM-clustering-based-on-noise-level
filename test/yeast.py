...
if __name__ == '__main__':
    ...
    X=np.loadtxt("yeast.txt") # dataset from https://cs.joensuu.fi/sipu/datasets/yeast.txt
    ...
    plt.savefig(r".\video\yeast_ori.png", dpi=dpi, bbox_inches='tight')
    ...
    ini_save_name = r".\video\yeast_ini.png"
    last_frame_name = r'.\video\yeast_last_frame_n_%d_alpha_0_%d.png' % (
        n_cluster, alpha_cut * 10)
    tmp_video_name = r'.\video\syeast_n_%d_alpha_%.1f_tmp.mp4' % (n_cluster, alpha_cut)
    video_save_newFps_name = r'.\video\yeast_n_%d_alpha_%.1f.mp4' % (n_cluster, alpha_cut)
