...
NPCM is run with $m_{ini}=10$ and several $\alpha\text{s}$. The result is shown in Fig.\ref{fig_close_cluster}. In Fig.\ref{fig_sub_close_0_2}, the two clusters merge due to the specification of a small noise level parameter $\alpha=0.2$. In Fig.\ref{fig_sub_close_0_3} and Fig.\ref{fig_sub_close_0_4}, when $\alpha$ increase to $0.3$ and $0.4$, the generated clusters overlap with each other just as the physical clusters. In Fig.\ref{fig_sub_close_0_5}, when $\alpha$ increase to $0.5$ and $0.6$, the influence of points in other clusters can be effectively reduced, therefore we get two well-separated clusters.
