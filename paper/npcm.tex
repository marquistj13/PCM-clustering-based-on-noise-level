...
\begin{document}
\author{\IEEEauthorblockN{Peixin~Hou\IEEEauthorrefmark{1}, Jiguang~Yue\IEEEauthorrefmark{1}, Hao~Deng\IEEEauthorrefmark{2}, and Shuguang~Liu\IEEEauthorrefmark{3}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}Department of Control Science and Engineering, Tongji University, Email: \{houpeixin, yuejiguang\}@tongji.edu.cn}
\IEEEauthorblockA{\IEEEauthorrefmark{2}School of Physics and Electronics, Henan University, Email: gavind@163.com}
\IEEEauthorblockA{\IEEEauthorrefmark{3}Department of Hydraulic Engineering, Tongji University, Email:  liusgliu@tongji.edu.cn}%
}
\maketitle
\begin{abstract}
...
\end{equation}
where $A_j$ and $n_j$ have the same meaning as in \eqref{apcm_eta_update}, and $\gamma$ should be small so that $\eta_j$ can be correctly estimated. In this paper, we choose $\gamma=0.01$. The rationale is that, the update process of $\eta_j$ should not be too sensitive to the point $\mathbf{x}_i$ near the boundary of clusters or to noisy points, and by so doing, the iteration times may also be reduced.
...
\section{Conclusion}
\label{sec-5}
In this paper, we develop a noise level based PCM clustering algorithm (NPCM). Specifically, NPCM addresses the background noise cluster problem encountered in APCM and UPCM and exploits the intuition from UPCM to eliminate the parameter $\sigma_v$ basing on $\eta_j$.
...
This research is supported by the Project supported by the National Key Technology Research and Development Program of the Ministry of Science and Technology of China (Grant No.2014BAL05B02)
\appendix
...
\begin{proof}
 We first calculate the geometrical locus of the points $\mathbf{x}$ having $u_2(\mathbf{x})>u_1(\mathbf{x})$, where $u_j(\mathbf{x})=\exp\left(-\frac{d_j^2(\mathbf{x})}{\gamma_j}\right)$ and $d_j(\mathbf{x})=\|\mathbf{x} - \boldsymbol{\theta}_j\|$, $j=1, 2$.
...
\end{equation*}
where $\epsilon\in(0,+\infty)$. Then we get $\|\mathbf{x} - \boldsymbol{\theta}_1\| > k'\|\mathbf{x} - \boldsymbol{\theta}_2\|$, and we have after some algebra
\begin{equation}
\label{hypersphere}
\|\mathbf{x}-\frac{k'^2\boldsymbol{\theta}_2-\boldsymbol{\theta}_1}{k'^2-1}\|^2 < \frac{k'^2}{(k'^2-1)^2}\|\boldsymbol{\theta}_2-\boldsymbol{\theta}_1\|^2\equiv r^2
\end{equation}
...
Note that $k'^2$ is larger than the $k$ used in the proof of APCM, therefore, convergence of NPCM is faster than APCM.
