...
The algorithm terminates when the change in $\theta_j$'s between two successive iterations is below than $10^{-5}$, i.e., $\|\boldsymbol{\theta}_j^{t}-\boldsymbol{\theta}_j^{t-1}\|\leq10^{-5}$.
*Experiment 1:* 
We can see that the clustering result and the estimated $\eta_j=0$ is not sensitive to the chosen of $\alpha$ for this dataset.
#+BEGIN_LaTeX
\begin{figure*}[tb]
\captionsetup[subfloat]{farskip=1pt,captionskip=1pt}% use this to reduce space between rows of subfloats, or between subfloat and the caption
   \centering
