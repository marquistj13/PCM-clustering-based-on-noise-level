...
The advantage of NPCM is that the parameters are intuitive to choose and NPCM does not need strong prior information (e.g., the cluster number) of the dataset.
We find that the update of $\eta_j$ is a positive feedback process and the adaptive $\sigma_v$ mechanism adopted in NPCM makes this positive feedback process more stronger, and a direct consequence of this fact is that NPCM has a faster convergence rate. 
